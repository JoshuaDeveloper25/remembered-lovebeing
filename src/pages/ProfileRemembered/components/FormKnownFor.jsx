...
const FormKnownFor = ({ isPending, bestKnownFor }) => {
      ...
      <article className="flex items-end gap-4">
        <label className="flex-[80%]">
          ...
          <input
            ...
            placeholder="Quality"
            name="quality"
            defaultValue={bestKnownInfoObject?.description}
          />
        </label>
...
const KnownFor = ({ knownFor, setBestKnownInfoObject }) => {
  ...
  const handleEditBestKnownFor = (e) => {
    ...
    const bestKnownInfo = {
      description: e?.target?.quality?.value?.trim(),
    };

    // Form validation
    if (!bestKnownInfo?.content)
  ...
  return (
    <li className="flex items-center mb-3 gap-4">
      ...
      <div className="flex items-center gap-3">
        <FaPencilAlt
          onClick={() =>
            setBestKnownInfoObject({ description: knownFor?.description })
          }
